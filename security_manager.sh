...
permitir_puerto() {
    echo 'quieres permitir un puerto nuevo [1] , o uno que ha sido bloqueado [2]?'
        case $opcion_puerto in
        1)
        ...
        2)
            ...
            read -p "Introduce el puerto que deseas permitir: " puerto
            read -p "¿Deseas permitir tráfico TCP o UDP? (tcp/udp): " tipo
            # Verificar si el puerto es válido
            if [[ ! "$puerto" =~ ^[0-9]+$ ]] || [ "$puerto" -lt 1 ] || [ "$puerto" -gt 65535 ]; then
              echo "Puerto no válido. Debe estar entre 1 y 65535."
              return
            fi

            # Eliminar la regla de bloqueo
             echo "Permitiendo el puerto $puerto..."
             sudo iptables -D INPUT -p $tipo --dport $puerto -j DROP
             echo "Puerto $puerto permitido correctamente."
        ;;
        *)
...
# Función para cerrar un puerto específico
cerrar_puerto() {
    read -p "Introduce el puerto que deseas cerrar: " 
    read -p "¿Deseas bloquear tráfico TCP o UDP? (tcp/udp): " tipo
    ...
    # Verificar si el puerto ya está cerrado (bloqueado)
    if sudo iptables -L INPUT -v -n | grep -q "$puerto"; then
        echo "El puerto $puerto ya está cerrado."
    else
        # Bloquear el puerto
        echo "Cerrando el puerto $puerto..."
        sudo iptables -A INPUT -p $tipo --dport $puerto -j DROP
        echo "Puerto $puerto cerrado correctamente."
    fi
}
...
#11 eliminar reglas
eliminar_reglas() {
    echo "Eliminación de reglas de firewall"
    echo "---------------------------------"
    
    # Mostrar las reglas actuales numeradas
    echo "Las reglas actuales son:"
    sudo iptables -L --line-numbers -n

    # Validación para cadena de entrada
    read -p "¿Quieres eliminar una regla de forma manual (1) o interactiva (2)? [1-2]: " opcion
    if [[ ! "$opcion" =~ ^[1-2]$ ]]; then
    ...
    if [ "$opcion" -eq 1 ]; then
        ...
        read -p "Introduce el número de la cadena (INPUT/FORWARD/OUTPUT): " cadena
        if [[ ! "$cadena" =~ ^(INPUT|FORWARD|OUTPUT)$ ]]; then
            echo "Opción no válida. Debes introducir INPUT, FORWARD o OUTPUT."
            return
        fi

        read -p "Introduce el número de la regla que deseas eliminar: " regla_num
        ...
        sudo iptables -D $cadena $regla_num
        echo "Se ha eliminado la regla número $regla_num en la cadena $cadena."

    elif [ "$opcion" -eq 2 ]; then
        # Eliminar de forma interactiva
        read -p "Introduce el número de la cadena (INPUT/FORWARD/OUTPUT): " cadena
        if [[ ! "$cadena" =~ ^(INPUT|FORWARD|OUTPUT)$ ]]; then
            echo "Opción no válida. Debes introducir INPUT, FORWARD o OUTPUT."
            return
        ...
        read -p "¿Quieres eliminar todas las reglas en esta cadena (s/n)? " respuesta
        if [[ ! "$respuesta" =~ ^(s|n)$ ]]; then
            echo "Respuesta no válida. Usa 's' para sí y 'n' para no."
            return
        fi

        if [ "$respuesta" == "s" ]; then
            # Eliminar todas las reglas en la cadena
            sudo iptables -F $cadena
            echo "Se han eliminado todas las reglas en la cadena $cadena."
        else
            # Listar reglas en la cadena especificada
            reglas=$(sudo iptables -L $cadena --line-numbers -n | grep -v "Chain" | awk '{print $1}')
            if [ -z "$reglas" ]; then
                echo "No hay reglas en la cadena $cadena."
                return
            fi

            # Mostrar reglas numeradas
            echo "Las reglas actuales en la cadena $cadena son:"
            sudo iptables -L $cadena --line-numbers -n
